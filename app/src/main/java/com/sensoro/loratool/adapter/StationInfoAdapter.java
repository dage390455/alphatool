...
public class StationInfoAdapter extends BaseAdapter {
            ...
            for (String firmWare : firmwareSet) {
                for (String key : mCacheStationInfoMap.keySet()) {
                    ...
                    if (stationInfo != null) {
                        ...
                        if ((firmWare.equals(firmwareVersion) || firmwareVersion.contains(firmWare) || isOther) && !tempFirmwareList.contains(stationInfo)) {
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ...
        if (mStationInfoList.size() > 0) {
            ...
            itemViewHolder.statusTimeTextView.setText(DateUtil.getDateDiffWithFormat(mContext, Long.parseLong(stationInfo.getSys().getPrevUpTime()), "MM-dd"));

            Drawable okDrawable = mContext.getResources().getDrawable(R.mipmap.ic_ok);
            okDrawable.setBounds(0, 0, okDrawable != null ? okDrawable.getMinimumWidth() : 0, okDrawable.getMinimumHeight());

            Drawable notDrawable = mContext.getResources().getDrawable(R.mipmap.ic_not);
            notDrawable.setBounds(0, 0, notDrawable != null ? notDrawable.getMinimumWidth() : 0, notDrawable.getMinimumHeight());

            Drawable usingDrawable = mContext.getResources().getDrawable(R.mipmap.ic_using);
            usingDrawable.setBounds(0, 0, usingDrawable != null ? usingDrawable.getMinimumWidth() : 0, usingDrawable.getMinimumHeight());
    ...
    public void refreshNew(SensoroStation sensoroStation, boolean isSearchStatus) {
        if (!mNearByStationMap.containsKey(sensoroStation.getSn())) {
            mNearByStationMap.put(sensoroStation.getSn(), sensoroStation);
        }
        if (isSearchStatus) {
            return;
        }
        ...
        for (int j = 0; j < mStationInfoList.size(); j++) {
            ...
            if (sensoroStation.getSn().equalsIgnoreCase(stationInfo.getSys().getSn())) {
                stationInfo.setSort(2);
                isContains = true;
                break;
            }
        ...
        if (!isContains) {
            if (isFitable(sensoroStation) && mCacheStationInfoMap.containsKey(sensoroStation.getSn())) {
                StationInfo stationInfo = mCacheStationInfoMap.get(sensoroStation.getSn());
                stationInfo.setSort(2);
                mStationInfoList.add(stationInfo);
    ...
    public void refreshGone(SensoroStation sensoroStation , boolean isSearchStatus) {
        if (mNearByStationMap.containsKey(sensoroStation.getSn())) {
            mNearByStationMap.remove(sensoroStation.getSn());
            for (int j = 0; j < mStationInfoList.size(); j++) {
                StationInfo stationInfo = mStationInfoList.get(j);
                if (sensoroStation.getSn().equalsIgnoreCase(stationInfo.getSys().getSn()) && isFilterNearby() && !isSearchStatus) {
